#提纲
#Part1  Bridge
       ehr_bridge.py 作为接口，用生成的timelines和qa jsonl来组装promot。   
       根据定义好的 type weights在 score_event函数中计算， 让后在selective retain 中按照topk保留事件，再按照时间重排
       render context函数开始渲染模块， build chat promot产生对话外壳， 这里要使用tokenizer
...
#启动内层实验室 (注意这里必须加 --allow-root，因为你是 root 用户)
...
keyin token   比如 http://ubuntu:8889/lab?token=ae6a36b549829dd62a0a305e9d6c213714b247ba6950800a
...
Triton 是 NVIDIA 开发的 GPU kernel 编程语言 + 编译器，允许用 Python 编写高性能 CUDA-like 算子，自动优化内存访问和并行策略。
  ...
  # Part3 vLLM
         ...
         深入 vLLM 的 block_manager（区块管理器）去抓取真实的碎片情况，同时把理论推导的 KV Cache 大小塞给 kv_cache_memory_mb
